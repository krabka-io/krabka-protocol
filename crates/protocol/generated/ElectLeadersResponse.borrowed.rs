...
impl<'de> DecodeBorrow<'de> for ElectLeadersResponse<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.replica_election_results = {
                let n = crate::primitives::array::get_array_len(buf, flex)?;
                let mut v = Vec::with_capacity(n);
                for _ in 0..n {
                    v.push(ReplicaElectionResult::decode_borrow(buf, version)?);
                }
                v
            };
        }
...
impl ElectLeadersResponse<'_> {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 0 {
            m.replica_election_results = vec![ReplicaElectionResult::populated(version)];
        }
        m
    }
}
...
impl Encode for ReplicaElectionResult<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            {
                crate::primitives::array::put_array_len(buf, (self.partition_result).len(), flex);
...
impl ReplicaElectionResult<'_> {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 0 {
            m.partition_result = vec![PartitionResult::populated(version)];
        }
        m
    }
}
