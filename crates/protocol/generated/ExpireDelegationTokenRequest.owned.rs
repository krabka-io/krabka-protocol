// AUTO-GENERATED by krabka-protocol-codegen against a9ce3221537b8653448750697915607dc7936cf3. Do not edit.

use bytes::{Buf, BufMut};

use crate::{
    Decode, Encode, ProtocolError, UnknownTaggedFields,
    primitives::{
        fixed::{get_i64, put_i64},
        string_bytes::{
            bytes_len, compact_bytes_len, get_bytes_owned, get_compact_bytes_owned, put_bytes,
            put_compact_bytes,
        },
    },
    tagged_fields::{WriteTaggedFields, read_tagged_fields, tagged_fields_len},
};
pub const API_KEY: i16 = 40;
pub const MIN_VERSION: i16 = 1;
pub const MAX_VERSION: i16 = 2;
pub const FLEXIBLE_MIN: i16 = 2;
#[inline]
#[must_use]
pub fn is_flexible(version: i16) -> bool {
    version >= FLEXIBLE_MIN
}
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct ExpireDelegationTokenRequest {
    pub hmac: ::bytes::Bytes,
    pub expiry_time_period_ms: i64,
    pub unknown_tagged_fields: UnknownTaggedFields,
}
impl Encode for ExpireDelegationTokenRequest {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        if !(MIN_VERSION..=MAX_VERSION).contains(&version) {
            return Err(ProtocolError::UnsupportedVersion {
                api_key: API_KEY,
                version,
            });
        }
        let flex = is_flexible(version);
        if version >= 0 {
            if flex {
                let () = put_compact_bytes(buf, &self.hmac);
            } else {
                let () = put_bytes(buf, &self.hmac);
            }
        }
        if version >= 0 {
            put_i64(buf, self.expiry_time_period_ms);
        }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
        }
        Ok(())
    }
    fn encoded_len(&self, version: i16) -> usize {
        let flex = is_flexible(version);
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex {
                compact_bytes_len(&self.hmac)
            } else {
                bytes_len(&self.hmac)
            };
        }
        if version >= 0 {
            n += 8;
        }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
        }
        n
    }
}
impl Decode<'_> for ExpireDelegationTokenRequest {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        if !(MIN_VERSION..=MAX_VERSION).contains(&version) {
            return Err(ProtocolError::UnsupportedVersion {
                api_key: API_KEY,
                version,
            });
        }
        let flex = is_flexible(version);
        let mut out = Self::default();
        if version >= 0 {
            out.hmac = if flex {
                get_compact_bytes_owned(buf)?
            } else {
                get_bytes_owned(buf)?
            };
        }
        if version >= 0 {
            out.expiry_time_period_ms = get_i64(buf)?;
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
        Ok(out)
    }
}
#[cfg(test)]
impl ExpireDelegationTokenRequest {
    #[must_use]
    pub fn populated(version: i16) -> Self {
        let mut m = Self::default();
        if version >= 0 {
            m.hmac = ::bytes::Bytes::from_static(b"x");
        }
        if version >= 0 {
            m.expiry_time_period_ms = 1i64;
        }
        m
    }
}
/// Default JSON payload matching `Self::default()` for JVM oracle differential testing.
/// Only includes fields valid for the given version.
#[must_use]
#[allow(unused_comparisons)]
pub fn default_json(_version: i16) -> ::serde_json::Value {
    let mut obj = ::serde_json::Map::new();
    obj.insert(
        "hmac".to_string(),
        ::serde_json::Value::String(String::new()),
    );
    obj.insert("expiryTimePeriodMs".to_string(), ::serde_json::json!(0));
    ::serde_json::Value::Object(obj)
}
impl crate::ProtocolRequest for ExpireDelegationTokenRequest {
    const API_KEY: i16 = API_KEY;
    const MIN_VERSION: i16 = MIN_VERSION;
    const MAX_VERSION: i16 = MAX_VERSION;
    const FLEXIBLE_MIN: i16 = FLEXIBLE_MIN;
    type Response = super::expire_delegation_token_response::ExpireDelegationTokenResponse;
}
