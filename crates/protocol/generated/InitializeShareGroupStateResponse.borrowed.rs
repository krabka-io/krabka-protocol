...
pub struct InitializeShareGroupStateResponse<'a> {
    pub results: Vec<InitializeStateResult<'a>>,
    ...
    ) -> crate::owned::initialize_share_group_state_response::InitializeShareGroupStateResponse
    {
        crate::owned::initialize_share_group_state_response::InitializeShareGroupStateResponse {
            results: (self.results)
                .iter()
                .map(InitializeStateResult::to_owned)
                .collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
impl Encode for InitializeShareGroupStateResponse<'_> {
    ...
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.results = {
                ...
                for _ in 0..n {
                    v.push(InitializeStateResult::decode_borrow(buf, version)?);
                }
                v
            };
        }
...
impl Encode for InitializeStateResult<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        let flex = version >= 0;
        if version >= 0 {
