...
`krabka-protocol` is the generated Kafka request/response and record-batch
codec. Krabka clients, broker code, and protocol tests all use it. It contains
...
- Optional compression, forwarded through `krabka-compression`.
...
or consumer behavior. Use `krabka-client-core` for typed RPC transport, and use
the higher-level client crates for application APIs.

The private `krabka-records-legacy` workspace crate holds the legacy v0/v1
...
# Ok::<(), krabka_protocol::ProtocolError>(())
```
...
- `gzip`, `snappy`, `lz4`, `zstd` - forward to `krabka-compression` codecs.
