...
mod tests {
    ...
    fn field(field_type: &str, default: Option<&str>) -> FieldSpec {
    ...
    #[test]
    fn scalar_differs_from_the_schema_default() {
        let cases = [
            ("int64", None, "1i64", "::serde_json::json!(1)"),
            ("int32", Some("1"), "2i32", "::serde_json::json!(2)"),
            ("int8", Some("-1"), "1i8", "::serde_json::json!(1)"),
            ("bool", None, "true", "::serde_json::Value::Bool(true)"),
            (
                "bool",
                Some("true"),
                "false",
        ...
        for (ty, default, rust, json) in cases {
            let f = field(ty, default);
            let got = scalar(ty, &f);
            assert!(
                (got.rust.as_str(), got.json.as_str()) == (rust, json),
                "{ty} {default:?}"
            );
        }
    }
    ...
    #[test]
    fn range_cond_checks_only_the_bounds_outer_does_not_share() {
        let cases = [
            (range(15, 18), range(0, 18), "version >= 15"),
            (range(0, 12), range(0, 18), "version <= 12"),
            (range(7, 18), range(4, 18), "version >= 7"),
            (range(4, 12), range(4, 18), "version <= 12"),
            (range(5, 9), range(0, 18), "(5..=9).contains(&version)"),
            (range(3, 3), range(0, 18), "version == 3"),
        ];
        for (r, outer, expected) in cases {
            assert!(range_cond(r, outer) == expected, "{r:?} in {outer:?}");
        }
    }
}
