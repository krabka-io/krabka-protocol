...
pub fn banner(schemas_version: &str) -> String {
    format!("{BANNER_PREFIX} against {schemas_version}. Do not edit.\n")
}

/// The opening bytes of every file the generator writes.
///
/// The generator shares its output directories with hand-written modules, for
/// example `crates/protocol/src/owned/fetch_response_plan.rs`. The regeneration
/// binary matches this prefix to tell one from the other before it deletes a
/// stale output.
pub const BANNER_PREFIX: &str = "// AUTO-GENERATED by krabka-protocol-codegen";
