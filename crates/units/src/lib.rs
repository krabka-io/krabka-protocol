...
//! function that takes two of them accepts them transposed and still compiles.
//! A value that crosses a module boundary in the wrong unit is a runtime bug and
//! not a compile error: seconds where milliseconds were expected, or bits where
//! bytes were expected. A unit written into the name shows the intent. It does
//! not enforce it.
//!
//! This crate gives those magnitudes real types, built on
//! [`uom`](https://docs.rs/uom): [`ByteSize`], [`ByteRate`], [`Time`],
//! [`Frequency`], and [`Ratio`]. Each one is a `uom` quantity, so unit
//! conversion is a method call and not a hand-written multiplication. `uom` also
...
//! and hertz. One storage type across all five dimensions is what makes
//! cross-dimension arithmetic usable, because `uom` only combines quantities that
//! share it. `f64` is also the only choice wide enough for both ends of the range
//! the broker needs. `uom`'s integer quantities store base units, so they cannot
//! represent a sub-second duration at all.
//!
...
//! and [`fmt`] writes them back. [`serde_units`] wires both into
//! `#[serde(with = ...)]`, so a config struct holds quantities and not integers.
//!
...
/// balance.
///
/// This is `uom`'s information dimension with `f64` storage. The base unit is
/// the byte, so [`ByteSizeExt::bytes_u64`](convert::ByteSizeExt::bytes_u64) is
/// exact for every value the wire can carry. Construct with [`bytes`],
/// [`kibibytes`], [`mebibytes`], or [`gibibytes`].
...
/// latency.
///
/// This is not an instant. See the crate docs. Construct with [`nanos`], [`micros`],
...
/// writing as a literal. Use [`convert`]'s `from_*` constructors for values that
/// need the full 64-bit range.
