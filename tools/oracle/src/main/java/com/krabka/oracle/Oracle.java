...
public final class Oracle {
    ...
    private static ObjectNode handle(JsonNode req) throws Exception {
        ...
        if (op.equals("record_batch_encode")) {
            ...
            if (req.has("messageName") && !req.get("messageName").isNull()) {
                ...
                Class<?> dataClass = messageClass(req.get("messageName").asText());
                msg = (Message) dataClass.getDeclaredConstructor().newInstance();
            } else {
                int apiKey = req.get("apiKey").asInt();
                ApiMessage apiMsg = isRequest
                        ? ApiKeys.forId(apiKey).messageType.newRequest()
                        : ApiKeys.forId(apiKey).messageType.newResponse();
                msg = apiMsg;
            }

            // Resolve the generated JsonConverter class next to the message
            // class, e.g.
            // org.apache.kafka.common.message.ApiVersionsRequestDataJsonConverter
            String converterName = msg.getClass().getName() + "JsonConverter";
            ...
            if (op.equals("encode")) {
                ...
                Message populated = (Message) readMethod.invoke(null, req.get("value"), version);
    ...
    private static Class<?> messageClass(String name) throws ClassNotFoundException {
        for (String format : MESSAGE_CLASS_FORMATS) {
            try {
                return Class.forName(String.format(format, name));
            } catch (ClassNotFoundException e) {
                // Try the next package.
            }
        }
        throw new ClassNotFoundException("no generated Kafka message class for " + name);
    }
